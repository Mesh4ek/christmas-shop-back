...
import imageBase64Map from './image-base64-map.json';
...
async function main() {
    ...
    try {
        const base64Map = imageBase64Map as Record<string, string>;
        ...
        const deletedOrderItems = await (prisma as any).orderItem.deleteMany({});
        console.log(`Deleted ${deletedOrderItems.count} order items`);
        
        const deletedOrders = await (prisma as any).order.deleteMany({});
        console.log(`Deleted ${deletedOrders.count} orders`);
        
        const deletedProducts = await (prisma as any).product.deleteMany({});
        ...
        const totalCount = await (prisma as any).product.count();
